...
func TestProxyServer_Integration(t *testing.T) {
	...
	backendListener, err := net.Listen("tcp", "127.0.0.1:11434")
	if err != nil {
	...
	proxyListener.Close() // Close it immediately so ProxyServer can bind to it
	...
	t.Run("Authorized Request - Proxy Forwards and Strips Token", func(t *testing.T) {
		...
		case backendReq := <-backendRequestChan:
			...
			if token := backendReq.Header.Get("x-api-key"); token != "" {
				t.Errorf("Expected 'x-api-key' to be stripped from the request, but found: %s", token)
			}
			// Verify path was successfully preserved
			if backendReq.URL.Path != "/v1/chat" {
				t.Errorf("Expected forwarded path '/v1/chat', got '%s'", backendReq.URL.Path)
			}
	...
	t.Run("Unauthorized Request - Middleware Blocks Execution", func(t *testing.T) {
		req, err := http.NewRequest(http.MethodGet, proxyAddr+"/v1/models", nil)
		if err != nil {
			t.Fatalf("Failed to create request: %v", err)
		}
